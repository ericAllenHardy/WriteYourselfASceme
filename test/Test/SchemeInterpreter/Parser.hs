...
module Test.SchemeInterpreter.Parser
  ( parserTests
  )
where
...
parserTests = do
  describe "Parser.parseString" $ it "parses special characters" $ property
    (id `parsesTo` String)
  describe "Parser.parseChar" $ it "parses special characters too" $ property
    ((\x -> "#\\" ++ [x]) `parsesTo` Char)
  describe "Parser.parseFloat" $ it "parses floats" $ property
    (printf "%.12f" `parsesTo` Float)
  describe "Parser.parseNumber" $ do
    it "parses hex" $ do
      property
        $ let f (NonNegative x) =
                readExpr ("#x" ++ showHex x "") == return (Number x)
          in  f
    it "parses octal" $ do
      property
        $ let f (NonNegative x) =
                readExpr ("#o" ++ showOct x "") == return (Number x)
          in  f
    it "parses binary" $ do
      property
        $ let f (NonNegative x) =
                readExpr ("#b" ++ showBin x "") == return (Number x)
          in  f
